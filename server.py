...
from agent_final import get_agent
...
@app.post("/run")
def run(m: RunRequest):
    user_input = m.prompt.strip()
    tools = [{
        "type": "function",
        "function": {
            "name": "execute_task",
    ...
    try:
        # 使用完整agent（集成Phase 1-4）
        agent = get_agent()
        result = agent.run(user_input)
        return {"status": "success", "result": result}
    except Exception as e:
        print(f"[SERVER] 执行失败: {e}")
        return {"status": "error", "result": str(e)}

@app.get("/events")
def events(last_id: int = 0):
    """返回增量事件（只返回last_id之后的事件）"""
    ...
    try:
        ...
        # 只返回last_id之后的事件
        new_events = [e for e in all_events if e["id"] > last_id]
