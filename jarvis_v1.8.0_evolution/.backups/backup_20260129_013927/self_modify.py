...
class SelfModifySession:
    ...
    def _discover_core_files(self) -> list:
        py_files = []
        for file in os.listdir(self.clawedbot_root):
...
'''
    ...
    def run_functional_test(self) -> Tuple[bool, str]:
        """运行功能测试：实际执行任务验证代码功能"""
        if not self.temp_dir:
            return False, "❌ 会话未启动"
        ...
        test_cases = [
            {"name": "基础文件读取", "goal": "读取/home/ubuntu/jarvis/VERSION.json文件的前50个字符", "expected_keywords": ["version", "1."], "timeout": 30},
            {"name": "工具调用", "goal": "列出/home/ubuntu/jarvis目录下的Python文件数量", "expected_keywords": [".py"], "timeout": 30},
        ]
        ...
        try:
            ...
            if not os.path.exists(server_script):
                return False, "❌ 临时目录中没有server.py"
            
            env = os.environ.copy()
            env['PORT'] = str(test_port)
            
            server_process = subprocess.Popen([sys.executable, server_script], cwd=self.temp_dir, stdout=subprocess.PIPE, stderr=subprocess.PIPE, env=env)
            time.sleep(5)
            
            try:
                health_resp = requests.get(f"http://localhost:{test_port}/health", timeout=5)
                if health_resp.status_code != 200:
                    return False, f"❌ 测试服务器启动失败"
            except Exception as e:
                return False, f"❌ 测试服务器启动失败: {str(e)}"
            
            results.append("✅ 测试服务器启动成功")
            
            for test_case in test_cases:
                try:
                    response = requests.post(f"http://localhost:{test_port}/run", json={"goal": test_case["goal"]}, timeout=test_case["timeout"])
                    if response.status_code == 200:
                        result_text = str(response.json())
                        if all(kw.lower() in result_text.lower() for kw in test_case["expected_keywords"]):
                            results.append(f"  ✅ {test_case['name']}: 通过")
                        else:
                            all_passed = False
                            results.append(f"  ❌ {test_case['name']}: 结果不符合预期")
                    else:
                        all_passed = False
                        results.append(f"  ❌ {test_case['name']}: HTTP {response.status_code}")
                except Exception as e:
                    all_passed = False
                    results.append(f"  ❌ {test_case['name']}: {str(e)[:100]}")
        finally:
            if server_process:
        ...
        status = "✅ 所有功能测试通过" if all_passed else "❌ 部分功能测试失败"
        self.test_results.append({"test": "functional_test", "passed": all_passed, "timestamp": datetime.now().isoformat()})
        return all_passed, f"{status}\n\n" + "\n".join(results)
    
    def run_regression_test(self) -> Tuple[bool, str]:
        """运行回归测试：确保修改没有破坏现有功能"""
        if not self.temp_dir:
            return False, "❌ 会话未启动"
        ...
        regression_tests = [
            {"name": "工具注册完整性", "code": "from executor import TOOLS\nexpected_tools = ['read_file', 'write_file', 'list_files']\nmissing = [t for t in expected_tools if t not in TOOLS]\nprint('PASS' if not missing else f'FAIL:缺少{missing}')"},
            {"name": "事件总线", "code": "from event_bus import emit\ntry:\n    emit('test', 'test')\n    print('PASS')\nexcept Exception as e:\n    print(f'FAIL:{e}')"}
        ]
        
        for test in regression_tests:
            test_code = f"import sys\nsys.path.insert(0, '{self.temp_dir}')\n{test['code']}"
            try:
                result = subprocess.run([sys.executable, "-c", test_code], capture_output=True, text=True, timeout=10, cwd=self.temp_dir)
                output = result.stdout.strip()
                if "PASS" in output:
                    results.append(f"  ✅ {test['name']}: 通过")
                else:
                    all_passed = False
                    results.append(f"  ❌ {test['name']}: {output if output else result.stderr[:100]}")
            except Exception as e:
                all_passed = False
                results.append(f"  ❌ {test['name']}: {str(e)[:100]}")
        
        status = "✅ 所有回归测试通过" if all_passed else "❌ 部分回归测试失败"
        self.test_results.append({"test": "regression_test", "passed": all_passed, "timestamp": datetime.now().isoformat()})
        return all_passed, f"{status}\n\n" + "\n".join(results)
    
    def handle_test_failure(self, test_name: str, error_message: str) -> Dict[str, Any]:
        """智能处理测试失败"""
        import re
        result = {"can_auto_fix": False, "fix_type": "unknown", "fix_actions": [], "analysis": ""}
        ...
        if "No module named" in error_message:
            ...
            if match:
                ...
                result["fix_type"] = "missing_module"
                result["fix_actions"] = [{"action": "create_file", "path": f"{module_name}.py", "content": f'"""\n{module_name} 模块\n"""\n'}]
                result["analysis"] = f"缺少模块 {module_name}，可以自动创建"
        elif "SyntaxError" in error_message:
            result["fix_type"] = "syntax_error"
            result["analysis"] = "语法错误，需要手动修复"
        else:
            result["analysis"] = f"未知错误: {error_message[:200]}"
        
        return result

    def run_all_tests(self, include_functional=False, include_regression=False) -> Tuple[bool, str]:
        """运行所有测试"""
        results = []
        all_passed = True
        ...
        if include_functional:
            results.append("\n### 3. 功能测试")
            passed, msg = self.run_functional_test()
            results.append(msg)
            if not passed:
                all_passed = False
        ...
        if include_regression:
            results.append("\n### 4. 回归测试")
            passed, msg = self.run_regression_test()
            results.append(msg)
            if not passed:
                all_passed = False
        
        summary = "✅ 所有测试通过" if all_passed else "❌ 部分测试失败"
    ...
    def apply_modifications(self, force: bool = False) -> Tuple[bool, str]:
        ...
        from execution_profile import get_current_profile
        profile_context = get_current_profile()
        
        if not force:
            failed_tests = [t for t in self.test_results if not t.get("passed", False)]
            if failed_tests:
                syntax_test = next((t for t in self.test_results if t.get("test") == "syntax_check"), None)
                if syntax_test and syntax_test.get("passed"):
                    if profile_context.is_development():
                        print("⚠️ DEV模式：语法检查通过，忽略导入测试失败，允许应用")
                    else:
                        return False, f"❌ 有 {len(failed_tests)} 个测试未通过，请先修复或使用 force=True 强制应用"
                else:
                    return False, f"❌ 语法检查未通过，无法应用修改"
