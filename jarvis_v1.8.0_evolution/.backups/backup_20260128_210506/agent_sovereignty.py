...
def quick_assess(action: str) -> Tuple[bool, str]:
    ...
    forbidden = [
        ...
        # 批量修改文件 - 绝对禁止
        ("批量修改", "禁止批量修改文件，必须逐个文件处理以确保安全"),
        ("批量替换", "禁止批量替换，必须逐个文件处理以确保安全"),
        ("batch modify", "Batch file modification is forbidden"),
        ("batch replace", "Batch replacement is forbidden"),
        ("sed -i", "禁止使用 sed -i 批量修改，风险不可控"),
        ("find -exec", "禁止使用 find -exec 批量操作"),
        ("xargs", "禁止使用 xargs 批量操作文件"),
        ("for f in *.", "禁止循环批量修改文件"),
        ("glob.glob", "禁止使用 glob 批量操作多个文件"),
        ("os.walk", "禁止使用 os.walk 批量操作文件"),
        ("pathlib.glob", "禁止使用 pathlib.glob 批量操作"),
    ]
