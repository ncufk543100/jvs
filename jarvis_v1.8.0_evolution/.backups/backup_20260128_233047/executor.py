...
def browse_url(params):
    ...
    try:
        ...
        req = urllib.request.Request(url, headers={
            'User-Agent': 'Mozilla/5.0 (compatible; ClawedBot/1.0)'
        })
...
def http_request(params):
    ...
    try:
        ...
        # 准备请求
        headers['User-Agent'] = headers.get('User-Agent', 'ClawedBot/1.0')
...
def web_search(params):
    ...
    try:
        ...
        req = urllib.request.Request(url, headers={'User-Agent': 'ClawedBot/1.0'})
...
TOOLS = {
    # 文件操作
    "scan_files": {"func": scan_files, "desc": "扫描项目文件"},
    "generate_project_map": {"func": generate_project_map, "desc": "生成项目地图"},
    "read_file": {"func": read_file, "desc": "读取文件内容"},
    "write_file": {"func": write_file, "desc": "写入文件"},
    "append_file": {"func": append_file, "desc": "追加内容到文件"},
    "list_files": {"func": list_files, "desc": "列出目录内容"},
    "search_files": {"func": search_files, "desc": "搜索文件内容"},
    
    # Shell 和代码执行
    "run_shell": {"func": run_shell, "desc": "执行 Shell 命令"},
    "run_python": {"func": run_python, "desc": "执行 Python 代码"},
    ...
    # 网络和HTTP
    "web_search": {"func": web_search, "desc": "网络搜索（DuckDuckGo）"},
    "browse_url": {"func": browse_url, "desc": "访问网页并提取内容"},
    "http_request": {"func": http_request, "desc": "发送HTTP请求（支持GET/POST等）"},
    "download_file": {"func": download_file, "desc": "下载文件到本地"},
}
...
def execute(command: str):
    ...
    if tool not in TOOLS:
        ...
        raise RuntimeError(f"未知工具: {tool}。可用工具: {available}")
