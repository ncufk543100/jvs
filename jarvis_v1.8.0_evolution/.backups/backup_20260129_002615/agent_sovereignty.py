...
from execution_profile import ExecutionProfile, ProfileContext, get_current_profile
...
def check_venv_for_command(command: str, project_root: str, profile_context: ProfileContext = None) -> Tuple[bool, str, Optional[str]]:
    """
    ...
    if profile_context.is_development():
        msg = f"""⚠️ DEV PROFILE: 虚拟环境检查已降级为告警

命令: {command}
项目目录: {project_root}

开发模式下，允许在没有虚拟环境的情况下执行。
此操作已记录为 DEV 模式操作。
...
def fast_policy_evaluator(
    ...
    """
    快速策略评估器（用于Development Profile）
    
    不调用LLM，使用简化的规则评估
    但仍然生成完整的Judgment，确保可审计
    """
    # 评估风险等级
    risk_level = assess_risk(action, context)
    
    # 快速判断
    can_proceed, reason = quick_assess(action)
    ...
    if not can_proceed:
        ...
        return AgentJudgment(
            ...
            reasoning=reason,
            conclusion="DEV PROFILE: 危险操作被拒绝",
            risks=[reason],
            alternatives=[],
            recommendation="此操作在任何模式下都不应执行",
            confidence=0.95  # 不是1.0，表示这是简化评估
        )
    
    # DEV模式：大部分操作直接通过，但标记清楚
    profile_name = profile_context.profile.value if profile_context else "dev"
    
    return AgentJudgment(
        judgment_type=JudgmentType.PROCEED,
        risk_level=risk_level,
        reasoning=f"DEV PROFILE: 使用快速评估策略，未进行完整LLM评估",
        conclusion=f"开发模式：快速执行（profile={profile_name}）",
        risks=["未进行完整风险评估"],
        alternatives=[],
        recommendation="继续执行，但已记录为DEV模式操作",
        confidence=0.7  # 明确低于1.0，表示这是简化评估
    )
...
def evaluate_action(
    ...
    # 如果是DEVELOPMENT模式，使用快速评估器
    if profile_context.is_development():
        judgment = fast_policy_evaluator(action, goal, context, profile_context)
        # 在judgment中添加profile标记
        return judgment
